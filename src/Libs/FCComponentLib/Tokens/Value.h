 ...
 */
struct FCComponentLibExport Numeric
{
 ...
 */
struct FCComponentLibExport Tuple
{
    struct Element
    {
 ...
 */
struct FCComponentLibExport Value: std::variant<Numeric, Base::Color, std::string, Tuple>
{
     ...
     *
     * Element-wise operations for tuples of equal size, scalar broadcast for Tuple x Numeric.
     * Delegates to Numeric operators for Numeric x Numeric.
     * @{
     */
...
const T& Tuple::get(const std::string& name) const
{
    ...
    if (!value->holds<T>()) {
        THROWM(
            ...
            fmt::format(
                "Argument '{}' must be {}, got {}", name, valueTypeName<T>(), value->toString()
            )
        );
    }
 ...
 * @endcode
 */
class FCComponentLibExport ArgumentParser
{
public:
