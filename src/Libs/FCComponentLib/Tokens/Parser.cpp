        ...
        if (peekNamedElement()) {
            ...
            if (pos < input.size() && input[pos] == ',') {
                ...
                if (!match(')')) {
                    THROWM(
                        Base::ParserError,
                        fmt::format("Expected ')', got '{}'", input[pos])
                    );
                }
            }
        }
...
std::unique_ptr<Expr> Parser::parseColor()
{
    ...
    const auto parseFunctionStyleColor = [&]() {
        ...
        if (!match(',')) {
            THROWM(
                Base::ParserError,
                fmt::format("Expected ',' after red, got '{}'", input[pos])
            );
        }
        int g = parseInt();
        if (!match(',')) {
            THROWM(
                Base::ParserError,
                fmt::format("Expected ',' after green, got '{}'", input[pos])
            );
        }
        int b = parseInt();
        ...
        if (hasAlpha) {
            if (!match(',')) {
                THROWM(
                    Base::ParserError,
                    fmt::format("Expected ',' after blue, got '{}'", input[pos])
                );
            }
            a = parseInt();
        }
        ...
        return std::make_unique<Color>(
            Base::Color(r / 255.0, g / 255.0, b / 255.0, a / 255.0)
        );
    };
...
std::unique_ptr<Expr> Parser::parseParameter()
{
    ...
    if (!match('@')) {
        THROWM(
            Base::ParserError,
            fmt::format("Expected '@' for parameter, got '{}'", input[pos])
        );
    }
    size_t start = pos;
...
std::unique_ptr<TupleLiteral>
Parser::parseTuple(std::optional<TupleLiteral::Element> firstElement)
{
    ...
    while (true) {
        ...
        if (!tuple->elements.empty() && !match(',')) {
            ...
            THROWM(
                Base::ParserError,
                fmt::format("Expected ',' or ')' in tuple, got '{}'", input[pos])
            );
        }
