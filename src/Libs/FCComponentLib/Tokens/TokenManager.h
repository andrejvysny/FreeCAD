...
// That macro uses inline const because some older compilers do not properly support constexpr
// for std::string. It should be changed into static constexpr once we migrate to newer compiler.
#define DEFINE_TOKEN(_name_, _defaultValue_) \
    static inline const FcComponents::Tokens::ParameterDefinition _name_ \
    { \
 ...
 * Example Usage:
 * @code{.cpp}
 * DEFINE_TOKEN(SomeParameter, Numeric { 10 });
 * DEFINE_TOKEN(TextColor, Base::Color(0.5F, 0.2F, 0.8F));
 * @endcode
 */
template<typename T>
 ...
 * - Act as a backend for parameter management, feeding the `TokenManager` with available
 *   parameter data.
 *
 * ### Metadata
 ...
 *
 * ### Notes on Usage
 * - Subclasses of `ParameterSource` (e.g., `InMemoryParameterSource`, `YamlParameterSource`)
 ...
 * - Parameters can be retrieved and manipulated globally through the `TokenManager`, which
 ...
 */
class FCComponentLibExport ParameterSource
{
public:
    ...
    ParameterSource(ParameterSource&&) = default;
    ParameterSource& operator=(ParameterSource&&) = default;
    ParameterSource(const ParameterSource&) = delete;
    ParameterSource& operator=(const ParameterSource&) = delete;
 ...
 */
class FCComponentLibExport InMemoryParameterSource: public ParameterSource
{
 ...
 */
class FCComponentLibExport YamlParameterSource: public ParameterSource
{
public:
    /**
 ...
 */
class FCComponentLibExport TokenManager
{
...
public:
    ...
    std::string replacePlaceholders(
        const std::string& expression, ResolveContext context = {}
    ) const;
...
ENABLE_BITMASK_OPERATORS(FcComponents::Tokens::ParameterSourceOption);
