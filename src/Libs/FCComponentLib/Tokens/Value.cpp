...
void Numeric::ensureEqualUnits(const Numeric& rhs) const
{
    if (unit != rhs.unit) {
        THROWM(
            Base::RuntimeError,
            fmt::format(
                "Units mismatch left expression is '{}', right expression is '{}'", unit, rhs.unit
            )
        );
    }
}
...
Value Value::operator*(const Value& rhs) const
{
    ...
    if (holds<Numeric>() && rhs.holds<Tuple>()) {
        return scalarBroadcast(
            rhs.get<Tuple>(), *this, [](const Value& elem, const Value& scalar) {
                return elem * scalar;
            }
        );
    }
...
Value Value::operator-() const
{
    ...
    if (holds<Tuple>()) {
        ...
        for (size_t i = 0; i < tuple.size(); ++i) {
            result.elements.emplace_back(
                tuple.elements[i].name, std::make_shared<const Value>(-tuple.at(i))
            );
        }
        return result;
    }
