...
FcCompassDial::FcCompassDial(QWidget* parent)
    : QWidget(parent)
    , m_markInterval(15)
    , m_defaultSize(75)
    , m_defaultMargin(10)
...
QSize FcCompassDial::sizeHint() const
{
    return m_rect.size();
}

QSize FcCompassDial::minimumSizeHint() const
{
    return QRect(0, 0, m_defaultSize, m_defaultSize).size();
}

void FcCompassDial::paintEvent(QPaintEvent* event)
{
    Q_UNUSED(event);
...
void FcCompassDial::drawWidget(QPainter& painter)
{
...
void FcCompassDial::drawBackground(QPainter& painter)
{
    painter.save();
...
void FcCompassDial::drawMarkings(QPainter& painter)
{
    CardinalMap CompassPointText({{0, "Y"},
                                  {45, "XY"},
                                  {90, "X"},
                                  {135, "X-Y"},
                                  {180, "-Y"},
    ...
    while (iDegree < 360) {
        if (iDegree % 45 == 0) {
            // Named direction (every 45 degrees)
...
void FcCompassDial::drawNeedle(QPainter& painter)
{
    painter.save();
    ...
    painter.drawPolygon(QPolygon({QPoint(0, sectionLineExtent),
    ...
    int needleExtent(40);
    painter.drawPolygon(QPolygon({QPoint(needleExtent, 0),
                                  QPoint(0, 5),
                                  QPoint(-15, 2),
                                  QPoint(-15, -2),
                                  QPoint(0, -5),
...
void FcCompassDial::setAngle(double newAngle)
{
    m_angle = fmod(360.0 - newAngle, 360.0);
    repaint();
}

void FcCompassDial::setSize(int newSize)
{
...
FC_REGISTER_COMPONENT(FcCompassDial, "Display", "Compass rose dial with needle")
