...
) const
{
    ...
    // Draw button-style background (same approach as FileCardDelegate)
    ...
    // Fetch thumbnail
    auto imageData =
        index.data(static_cast<int>(Start::DisplayedFilesModelRoles::image)).toByteArray();
    ...
    // Draw filename
    auto baseName =
        index.data(static_cast<int>(Start::DisplayedFilesModelRoles::baseName)).toString();
    QString elidedName = fm.elidedText(baseName, Qt::ElideRight, textWidth);
    painter->drawText(nameRect, Qt::AlignLeft | Qt::AlignVCenter, elidedName);

    // Draw file size in lighter, smaller font
    auto fileSize =
        index.data(static_cast<int>(Start::DisplayedFilesModelRoles::size)).toString();
