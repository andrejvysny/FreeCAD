...
{
    // Parse "X.Y" or "X.Y.Z" from ExeVersion and build the wiki URL
    QString version =
        QString::fromStdString(App::Application::Config()["ExeVersion"]);
    QStringList parts = version.split(QLatin1Char('.'));
    QString majorMinor = parts.size() >= 2
        ? parts.at(0) + QLatin1Char('_') + parts.at(1)
        : version;
...
QString LearnLinksWidget::whatsNewLabel()
{
    ...
    // tr() cannot be called from a static method; label is assembled in constructor
    return majorMinor;
}
